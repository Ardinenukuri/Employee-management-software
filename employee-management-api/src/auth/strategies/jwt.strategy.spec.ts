...
describe('JwtStrategy', () => {
  ...
  beforeEach(() => {
    ...
    strategy = new JwtStrategy(repo, config);
  });
  ...
  it('should throw UnauthorizedException if user not found', async () => {
    ...
    await expect(strategy.validate({ email: 'none' })).rejects.toThrow(
  ...
  it('should throw error if JWT_SECRET is missing (undefined)', () => {
    ...
    expect(
      () => new JwtStrategy({ findOneBy: jest.fn() } as any, cfg as any),
    ).toThrow('JWT secret not found in environment variables.');
  });
